...
export function ContactButtons({ phone, whatsapp, title, listingId }: ContactButtonsProps) {
  ...
  const whatsappUrl = `https://wa.me/${whatsappNumber.replace("+", "")}?text=${encodeURIComponent(
    `Hi, I'm interested in this property on PropFind: ${title}`
  )}`;
  ...
  const trackContact = (type: "phone" | "whatsapp") => {
    if (listingId) {
      ...
      fetch(`/api/listings/${listingId}/contact`, {
        ...
        body: JSON.stringify({ contactType: type, visitorId }),
      }).catch(() => {});
    }
  };
